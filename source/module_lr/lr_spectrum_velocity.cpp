...
namespace LR
{
    ...
    ModuleBase::Vector3<T> LR::LR_Spectrum<T>::cal_transition_dipole_istate_velocity_R(const int istate, const TD_current& vR)
    {
        ...
        for (int i = 0; i < 3; i++)
        {
            ...
            {
                trans_dipole[i] += LR_Util::dot_R_matrix(*vR.get_current_term_pointer(i), *DM_trans.get_DMR_pointer(is + 1)) * fac;
    ...
    void LR::LR_Spectrum<T>::test_transition_dipoles_velocity_ks(const double* const ks_eig)
    {
        ...
        for (int istate = 0;istate < nstate;++istate)
        {
            ...
            for (int i = 0; i < 3; i++)
            {
                for (int is = 0;is < this->nspin_x; ++is)
                {
                    tmp_trans_dipole[i] += LR_Util::dot_R_matrix(*vR.get_current_term_pointer(i), *DM_trans.get_DMR_pointer(is + 1)) * ModuleBase::IMAG_UNIT;
                }   // end for spin_x, only matter in open-shell system
            }   // end for direction
