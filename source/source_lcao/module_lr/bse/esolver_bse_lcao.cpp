...
ESolver_BSE<T, TR>::ESolver_BSE(const Input_para& inp, UnitCell& ucell) :
    LR::ESolver_LR<T, TR>(inp, ucell, LR::DeferredESolverLRInit{})
{
    ModuleBase::TITLE("ESolver_BSE", "ESolver_BSE(from scratch)");
    ModuleBase::timer::start("ESolver_BSE", "constructor");
    // xc kernel
    ...
    ModuleBase::timer::end("ESolver_BSE", "constructor");
}
...
void ESolver_BSE<T, TR>::runner(UnitCell& ucell, const int istep)
{
    ...
    if (this->input.lr_solver == "elpa")
    {
        ...
        else
        {
            ...
            this->lri_init();
            HamiltBSE<T> bse_matrix(this->nspin, this->nbasis, this->nocc, this->nvirt, this->ucell,
